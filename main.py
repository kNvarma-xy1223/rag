...
<style>
  :root {
    ...
    --mono:'JetBrains Mono',monospace;--sans:'Barlow',sans-serif;--condensed:'Barlow Condensed',sans-serif;
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .progress-pct{font-family:var(--mono);font-size:11px;color:var(--text-3);float:right}

  /* Streaming answer cursor blink */
  ...
  /* Perplexity-style citation badges */
  ...
  .chat-avatar{width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0;font-weight:700;font-family:var(--condensed)}
  .chat-msg.user .chat-avatar{background:var(--accent);color:#fff}
  .chat-msg.assistant .chat-avatar{background:var(--surface-2);border:1px solid var(--border);color:var(--text-2);font-size:16px}
...
<header>
  <div style="display:flex;align-items:center;gap:10px">
    ...
    <div>
      ...
      <div class="brand-sub">OpenAI text-embedding-3-large &middot; Cohere embed-v4.0 &middot; Pinecone &middot; GPT-5.4-pro</div>
    </div>
  </div>
  ...
  <div class="two-col">
    <div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <p style="font-size:13px;color:var(--text-2);line-height:1.7;margin-bottom:12px">Upload PDF or CSV files to index into Pinecone. Use the Query or Chat tabs to get answers.</p>
    ...
    <div>
      <div class="card">
        <div class="card-header"><span class="card-title">Ingestion Results</span><span id="ingestStatus"></span></div>
        <div class="card-body" id="ingestResults">
          <div class="empty-state"><div class="empty-icon">&#8679;</div><div>Upload a document to begin</div></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="two-col">
    <div>
      <div class="card">
        ...
        <div class="card-body">
          <div class="field"><label>Question</label><textarea id="queryText" rows="3" placeholder="Ask in English or Spanish..."></textarea></div>
          <div class="inline-row">
            <div class="field"><label>Embedding Model</label><select id="queryModel"><option value="openai">OpenAI</option><option value="cohere">Cohere</option><option value="both">Both (side-by-side)</option></select></div>
            <div class="field"><label>Top-K</label><input type="number" id="queryTopK" value="5" min="1" max="20"></div>
          </div>
          <div class="field"><label>Filter: source_type</label><select id="queryFilterType"><option value="">Any</option><option value="pdf">PDF only</option><option value="csv">CSV only</option></select></div>
          <div class="field"><label>Score Threshold</label><input type="number" id="queryThreshold" value="0.0" min="0" max="1" step="0.05"></div>
          <button class="btn btn-primary btn-full" id="queryBtn">Ask Question</button>
        </div>
      </div>
    </div>
    <div id="queryOutput">
      <div class="empty-state card" style="padding:40px"><div class="empty-icon">&#128172;</div><div>Enter a question and click Ask</div></div>
    </div>
  </div>
</div>
  ...
  <div class="card" style="margin-bottom:16px">
    ...
    <div class="card-body">
      <div style="display:flex;gap:12px;align-items:flex-end">
        <div class="field" style="margin:0;flex:1"><label>Query</label><input type="text" id="compareQuery" placeholder="Query both models simultaneously..."></div>
        <div class="field" style="margin:0;width:100px"><label>Top-K</label><input type="number" id="compareTopK" value="5" min="1" max="20"></div>
        <button class="btn btn-primary" id="compareBtn">Compare</button>
      </div>
    </div>
  </div>
  <div id="compareOutput"><div class="empty-state card" style="padding:40px"><div class="empty-icon">&#9878;</div><div>Run a comparison to see results side by side</div></div></div>
</div>
...
<div class="tab-panel" id="tab-chat">
  <div class="chat-layout">
    <div class="chat-toolbar">
      ...
      <select id="chatModel"><option value="openai">OpenAI</option><option value="cohere">Cohere</option></select>
    ...
    <div class="chat-messages" id="chatMessages">
      <div class="chat-msg assistant">
        ...
        <div class="chat-msg-wrap">
          <div class="chat-bubble">Hello! I'm your RAG assistant. Ask me anything about your ingested documents — I'll cite my sources inline like Perplexity.</div>
        </div>
      </div>
    </div>
...
<div class="tab-panel" id="tab-evaluate">
  <div class="two-col">
    <div>
      <div class="card">
        <div class="card-header"><span class="card-title">Benchmark Settings</span></div>
        <div class="card-body">
          <div class="field"><label>Embedding Model</label><select id="evalModel"><option value="openai">OpenAI</option><option value="cohere">Cohere</option><option value="both">Both (Compare Side-by-Side)</option></select></div>
          <div class="field"><label>K (top results)</label><input type="number" id="evalK" value="5" min="1" max="20"></div>
          <button class="btn btn-secondary btn-full" id="loadBenchmarkBtn" style="margin-bottom:10px">Load Sample Benchmark</button>
          <div class="field"><label>Queries JSON</label><textarea id="evalQueries" rows="8" placeholder='[{"query":"...", "relevant_doc_ids":["..."]}, ...]' style="font-family:var(--mono);font-size:11px"></textarea></div>
          <button class="btn btn-primary btn-full" id="evalBtn">Run Benchmark</button>
        </div>
      </div>
    </div>
    <div id="evalOutput"><div class="empty-state card" style="padding:40px"><div class="empty-icon">&#128202;</div><div>Run benchmark to see evaluation metrics</div></div></div>
  </div>
</div>

<!-- COLLECTIONS -->
<div class="tab-panel" id="tab-collections">
  <div id="collectionsOutput"><div class="empty-state card" style="padding:40px"><div class="empty-icon">&#128452;</div><div>Loading collection stats...</div></div></div>
</div>
...
function apiFetch(path, opts) {
  return fetch(API + path, opts || {}).then(function(res) {
    if (!res.ok) return res.json().catch(function(){ return {detail: res.statusText}; }).then(function(e){ throw new Error(e.detail || JSON.stringify(e)); });
    return res.json();
  });
}
...
function renderFileList() {
  if (!selectedFiles.length) { fileListDisplay.style.display='none'; ingestBtn.disabled=true; return; }
  fileListDisplay.style.display = 'block'; ingestBtn.disabled = false;
  fileListDisplay.innerHTML = selectedFiles.map(function(f,i) {
    return '<div class="file-list-item"><span>&#128196; ' + esc(f.name) + '</span><span class="file-size">' + fmtBytes(f.size) + '</span><button class="file-remove" data-idx="' + i + '">&#10005;</button></div>';
  }).join('');
...
function addFiles(files) {
  Array.from(files).forEach(function(f) {
    if (!selectedFiles.some(function(s){ return s.name===f.name && s.size===f.size; })) selectedFiles.push(f);
  });
  renderFileList();
}
...
dropZone.addEventListener('drop', function(e){ e.preventDefault(); dropZone.classList.remove('drag-over'); if(e.dataTransfer.files.length) addFiles(e.dataTransfer.files); });
...
ingestBtn.addEventListener('click', async function() {
    ...
    try {
      ...
      var result = await new Promise(function(resolve, reject) {
        fetch('/api/ingest', {method:'POST', body:fd}).then(function(resp) {
          if (!resp.ok) {
            resp.json().catch(function(){ return {detail:resp.statusText}; }).then(function(e){ reject(new Error(e.detail||JSON.stringify(e))); });
            return;
          }
              ...
              for (var j=0; j<lines.length; j++) {
                ...
                try {
                  ...
                  if (ev.type === 'progress') {
                    ...
                    var bar2 = document.getElementById(pid); if (bar2) bar2.style.width = '100%';
                    resolve(ev);
  ...
  selectedFiles=[]; fileInput.value=''; renderFileList(); setLoading(ingestBtn, false);
});

function renderOneIngest(d) {
  var rows = [['Type', d.source_type.toUpperCase()], ['Raw documents', d.raw_documents], ['Semantic chunks', d.chunks]];
  if (d.indexed.openai !== undefined) rows.push(['<span class="model-badge openai">OpenAI</span> vectors', d.indexed.openai]);
  if (d.indexed.cohere !== undefined) rows.push(['<span class="model-badge cohere">Cohere</span> vectors', d.indexed.cohere]);
  return '<div style="border:1px solid var(--border);border-radius:var(--radius-sm);margin-bottom:10px;overflow:hidden">'
    ...
    + '<span style="margin-left:auto;font-family:var(--mono);font-size:11px;color:var(--success);background:var(--success-bg);border:1px solid #86efac;border-radius:99px;padding:1px 8px">&#10003; done</span></div>'
    + '<div style="padding:6px 14px">' + rows.map(function(r){ return '<div class="stat-row"><span class="stat-key">' + r[0] + '</span><span class="stat-val">' + r[1] + '</span></div>'; }).join('') + '</div></div>';
}

function renderMultiIngest(results, errors) {
  var total = results.length + errors.length;
  var summary = total > 1 ? '<div style="margin-bottom:14px;padding:10px 14px;background:var(--surface-2);border:1px solid var(--border);border-radius:var(--radius-sm);font-family:var(--condensed);font-size:13px;color:var(--text-2)">' + results.length + ' of ' + total + ' files indexed successfully</div>' : '';
  var s = results.map(renderOneIngest).join('');
  var e = errors.map(function(e){ return '<div style="border:1px solid #fca5a5;border-radius:var(--radius-sm);margin-bottom:10px;padding:10px 14px;background:var(--danger-bg);font-size:12px;color:var(--danger)">&#128196; ' + esc(e.name) + ': ' + esc(e.error) + '</div>'; }).join('');
...
function queryPayload(model) {
  ...
  return {query: document.getElementById('queryText').value.trim(), embedding_model: model,
...
document.getElementById('queryBtn').addEventListener('click', async function() {
  ...
  if (model === 'both') {
    // "Both" mode: run two non-streaming requests in parallel and render side-by-side
    document.getElementById('queryOutput').innerHTML = '<div class="empty-state card" style="padding:40px"><div class="empty-icon">&#9203;</div><div>Querying both models...</div></div>';
    try {
      // /api/query now returns SSE — use a helper that collects the full answer
      var results = await Promise.all([collectStream(queryPayload('openai')), collectStream(queryPayload('cohere'))]);
  ...
  // Single model: stream tokens live into the answer box
      ...
      for (var i=0; i<lines.length; i++) {
        ...
        try {
          ...
          if (ev.type === 'citations') {
            citations = ev.citations; ret_ms = ev.retrieval_latency_ms;
            metaEl.innerHTML = mBadge + '<span class="latency-pill ' + latCls(ret_ms) + '">Retrieval: ' + ret_ms + 'ms</span><span class="latency-pill">Generating...</span>';
            if (citations.length) {
...
// Collect a full SSE stream into a pseudo-response object (used for "both" mode)
async function collectStream(payload) {
  ...
  var reader = resp.body.getReader(), decoder = new TextDecoder(), buf = '';
  var out = {embedding_model:payload.embedding_model, answer:'', citations:[], retrieval_latency_ms:0, generation_latency_ms:0, tokens_used:null};
  while (true) {
    var chunk = await reader.read();
    if (chunk.done) break;
    buf += decoder.decode(chunk.value, {stream:true});
...
function renderCitations(citations, uid) {
  return citations.map(function(c) {
    return '<div class="citation-item" id="cite-' + uid + '-' + c.index + '">'
      ...
      + '<div class="citation-content"><div class="citation-header"><span class="citation-source">' + esc(c.source) + '</span><span class="citation-loc">' + esc(c.location) + '</span><span class="score-val" style="color:' + scoreColor(c.score) + '">' + c.score.toFixed(4) + '</span></div>'
      + '<div class="citation-preview">' + esc(c.preview) + '</div></div></div>';
  }).join('');
}
...
function renderQuery(d, uid) {
  ...
  return '<div style="display:flex;flex-direction:column;gap:14px">'
    + '<div class="card"><div class="card-header"><span class="card-title">Answer</span>'
    + '<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">' + mBadge
    ...
    + (cites ? '<div class="card"><div class="card-header"><span class="card-title">Sources (' + d.citations.length + ')</span></div><div class="card-body" style="padding-top:10px">' + cites + '</div></div>' : '')
    + '</div>';
}
...
function renderBothQuery(oa, co) {
  return '<div style="display:flex;flex-direction:column;gap:12px">'
    ...
    + renderQuery(oa,'oa') + renderQuery(co,'co') + '</div></div>';
}

function renderChunk(chunk, rank, uid) {
  var meta = chunk.metadata || {}, source = meta.source || 'unknown';
  var loc = meta.page ? '<span class="chunk-meta">page ' + meta.page + '</span>'
    : meta.row_start ? '<span class="chunk-meta">rows ' + meta.row_start + '-' + meta.row_end + '</span>'
    : meta.doc_type ? '<span class="chunk-meta">' + meta.doc_type + '</span>' : '';
  if (meta.language) loc += ' <span class="chunk-meta">' + meta.language + '</span>';
  var sb = '<div class="score-bar-wrap" style="min-width:80px;flex:1;max-width:120px"><div class="score-bar"><div class="score-bar-fill" style="width:' + (chunk.score*100) + '%;background:' + scoreColor(chunk.score) + '"></div></div><span class="score-val">' + chunk.score.toFixed(4) + '</span></div>';
  var id = 'cb-' + uid + '-' + rank;
  return '<div class="chunk-item"><div class="chunk-header"><span class="chunk-rank">#' + rank + '</span><span class="chunk-source">' + esc(source) + '</span>' + loc + '<div style="margin-left:auto">' + sb + '</div></div><div class="chunk-body" id="' + id + '">' + esc(chunk.text) + '</div><span class="chunk-expand" data-target="' + id + '">Show more</span></div>';
}
...
function attachExpandHandlers() {
  document.querySelectorAll('.chunk-expand').forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      if (t.classList.contains('expanded')) { t.classList.remove('expanded'); btn.textContent='Show more'; }
      else { t.classList.add('expanded'); btn.textContent='Show less'; }
    });
  });
}
...
function attachCiteHandlers() {
  document.querySelectorAll('.cite-sup').forEach(function(sup) {
    sup.addEventListener('click', function() {
      ...
      if (!citeEl) {
        // fallback: find any citation item with matching number
...
document.getElementById('compareBtn').addEventListener('click', async function() {
  ...
  setLoading(btn, true);
  document.getElementById('compareOutput').innerHTML = '<div class="empty-state card" style="padding:40px"><div class="empty-icon">&#9203;</div><div>Querying both models...</div></div>';
  try {
    var data = await apiFetch('/api/compare', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:query,top_k:parseInt(document.getElementById('compareTopK').value)})});
    document.getElementById('compareOutput').innerHTML = renderCompare(data);
    attachExpandHandlers();
  } catch(e) { document.getElementById('compareOutput').innerHTML = '<div class="error-box">' + esc(e.message) + '</div>'; showToast(e.message,'error'); }
  setLoading(btn, false);
});

function renderCompare(d) {
  var c = d.comparison, op = Math.round(c.result_overlap*100);
  var summary = '<div class="card" style="margin-bottom:16px"><div class="card-header"><span class="card-title">Comparison Summary</span></div><div class="card-body"><div class="metric-grid">'
    ...
    + '</div><div><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px"><span style="color:var(--text-2);font-family:var(--condensed);font-weight:600;text-transform:uppercase;letter-spacing:.5px">Result Overlap</span><span style="font-family:var(--mono)">' + op + '%</span></div>'
    + '<div class="overlap-bar"><div class="overlap-fill" style="width:' + op + '%"></div></div>'
    + '<div style="font-size:11px;color:var(--text-3);margin-top:4px;font-family:var(--mono)">' + c.openai_total + ' OpenAI &middot; ' + c.cohere_total + ' Cohere &middot; Jaccard: ' + c.result_overlap.toFixed(4) + '</div>'
    + '</div></div></div>';
  var oc = (d.openai.results||[]).map(function(c,i){ return renderChunk(c,i+1,'oa'); }).join('');
  var cc = (d.cohere.results||[]).map(function(c,i){ return renderChunk(c,i+1,'co'); }).join('');
  return summary + '<div class="compare-grid">'
    ...
    + '<div class="card"><div class="card-header"><span style="font-family:var(--condensed);font-weight:600;font-size:14px;color:var(--cohere)">Cohere embed-v4.0</span><span class="latency-pill ' + latCls(c.cohere_latency_ms) + '">' + c.cohere_latency_ms + 'ms</span></div><div class="card-body" style="padding-top:10px">' + (cc||'<div class="empty-state">No results</div>') + '</div></div>'
    + '</div>';
}
...
function appendMsg(role, content, citations, metaStr) {
  ...
  var bubble = role === 'user'
    ...
    : '<div class="chat-bubble">' + renderWithCites(content, citations, 'chat' + Date.now()) + '</div>';
  var sources = (role === 'assistant' && citations && citations.length)
    ? '<div class="chat-sources">' + citations.map(function(c){
        return '<span class="chat-source-chip" title="' + esc(c.preview) + '">&#128196; ' + esc(c.source) + ' &middot; ' + esc(c.location) + ' <span style="opacity:.5">' + c.score.toFixed(2) + '</span></span>';
      }).join('') + '</div>' : '';
  var meta = metaStr ? '<div class="chat-meta">' + metaStr + '</div>' : '';
  el.innerHTML = '<div class="chat-avatar">' + avatar + '</div><div class="chat-msg-wrap">' + bubble + sources + meta + '</div>';
  msgs.appendChild(el);
...
function showTyping() {
  var msgs = document.getElementById('chatMessages');
  var el = document.createElement('div');
  el.className = 'chat-msg assistant'; el.id = 'chatTyping';
  el.innerHTML = '<div class="chat-avatar">&#129302;</div><div class="chat-msg-wrap"><div class="chat-bubble"><div class="typing-dot"><span></span><span></span><span></span></div></div></div>';
...
async function sendChat() {
  ...
  // Create a streaming assistant bubble immediately
  var msgs = document.getElementById('chatMessages');
  var msgEl = document.createElement('div');
  msgEl.className = 'chat-msg assistant';
  var sid = 'cs-' + Date.now();
  msgEl.innerHTML = '<div class="chat-avatar">&#129302;</div>'
  ...
  try {
    var resp = await fetch('/api/chat', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({
        query: query, history: chatHistory.slice(-6),
      ...
      for (var i=0; i<lines.length; i++) {
        ...
        try {
          ...
          if (ev.type === 'citations') {
            ...
            var meta = '<div class="chat-meta">' + modelLabel + ' &middot; ' + ret_ms + 'ms ret &middot; ' + ev.generation_latency_ms + 'ms gen ' + acc + '</div>';
...
document.getElementById('chatClearBtn').addEventListener('click', function() {
  chatHistory = [];
  document.getElementById('chatMessages').innerHTML = '<div class="chat-msg assistant"><div class="chat-avatar">&#129302;</div><div class="chat-msg-wrap"><div class="chat-bubble">Chat cleared. Ask me anything about your documents!</div></div></div>';
  showToast('Chat cleared','info');
});

// --- EVALUATE ---
document.getElementById('loadBenchmarkBtn').addEventListener('click', async function() {
  try { var data = await apiFetch('/api/benchmark-sample'); document.getElementById('evalQueries').value = JSON.stringify(data, null, 2); showToast('Loaded ' + data.length + ' benchmark queries','success'); }
  catch(e) { showToast('Could not load benchmark queries','error'); }
});

document.getElementById('evalBtn').addEventListener('click', async function() {
  var raw = document.getElementById('evalQueries').value.trim();
  ...
  var model = document.getElementById('evalModel').value;
  var k = parseInt(document.getElementById('evalK').value);
  setLoading(btn, true);
  document.getElementById('evalOutput').innerHTML = '<div class="empty-state card" style="padding:40px"><div class="empty-icon">&#9203;</div><div>Running benchmark...</div></div>';
  try {
    if (model === 'both') {
      var data = await apiFetch('/api/benchmark-compare', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({benchmark_queries:queries,embedding_model:'openai',k:k})});
      document.getElementById('evalOutput').innerHTML = renderEvalCompare(data, k, queries.length);
    } else {
      var data = await apiFetch('/api/benchmark', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({benchmark_queries:queries,embedding_model:model,k:k})});
      document.getElementById('evalOutput').innerHTML = renderEval(data);
    }
  } catch(e) { document.getElementById('evalOutput').innerHTML = '<div class="error-box">' + esc(e.message) + '</div>'; showToast(e.message,'error'); }
  setLoading(btn, false);
});
...
function renderEval(d) {
  ...
  var mBadge = '<span class="model-badge ' + model + '">' + (model==='openai'?'OpenAI':'Cohere') + '</span>';
  var cards = [['Precision@'+d.k, a.avg_precision_at_k.toFixed(4)],['Recall@'+d.k, a.avg_recall_at_k.toFixed(4)],['MRR', a.avg_mrr.toFixed(4)],['NDCG@'+d.k, a.avg_ndcg_at_k.toFixed(4)],['Avg Latency', a.avg_latency_ms+'ms'],['P95 Latency', a.p95_latency_ms+'ms']].map(function(m){ return '<div class="metric-card"><div class="metric-value">' + m[1] + '</div><div class="metric-label">' + m[0] + '</div></div>'; }).join('');
  var rows = (d.per_query||[]).map(function(r,i){ return '<tr><td>'+(i+1)+'</td><td style="max-width:220px;word-break:break-word">'+esc(r.query)+'</td><td class="mono-val">'+r.precision_at_k.toFixed(4)+'</td><td class="mono-val">'+r.recall_at_k.toFixed(4)+'</td><td class="mono-val">'+r.mrr.toFixed(4)+'</td><td class="mono-val">'+r.ndcg_at_k.toFixed(4)+'</td><td class="mono-val">'+r.latency_ms+'ms</td></tr>'; }).join('');
  return '<div style="display:flex;flex-direction:column;gap:16px"><div class="card"><div class="card-header"><span class="card-title">Aggregate Metrics</span><div style="display:flex;gap:8px;align-items:center">'+mBadge+'<span class="latency-pill">K='+d.k+'</span><span class="latency-pill">'+d.num_queries+' queries</span></div></div><div class="card-body"><div class="metric-grid">'+cards+'</div></div></div><div class="card"><div class="card-header"><span class="card-title">Per-Query Results</span></div><div style="overflow-x:auto"><table class="data-table"><thead><tr><th>#</th><th>Query</th><th>Precision@K</th><th>Recall@K</th><th>MRR</th><th>NDCG@K</th><th>Latency</th></tr></thead><tbody>'+rows+'</tbody></table></div></div></div>';
}

function renderEvalCompare(data, k, numQ) {
  var oa = data.openai, co = data.cohere;
  var metrics = [['Precision@'+k,'avg_precision_at_k'],['Recall@'+k,'avg_recall_at_k'],['MRR','avg_mrr'],['NDCG@'+k,'avg_ndcg_at_k']];
  var rows = metrics.map(function(m) {
    var ov = oa.aggregate[m[1]], cv = co.aggregate[m[1]];
    var w = ov > cv ? 'openai' : cv > ov ? 'cohere' : '';
    return '<tr><td style="font-family:var(--condensed);font-weight:600">'+m[0]+'</td><td class="mono-val" style="color:'+(w==='openai'?'var(--openai)':'var(--text-2)')+'"><b>'+ov.toFixed(4)+'</b>'+(w==='openai'?' &#10003;':'')+'</td><td class="mono-val" style="color:'+(w==='cohere'?'var(--cohere)':'var(--text-2)')+'"><b>'+cv.toFixed(4)+'</b>'+(w==='cohere'?' &#10003;':'')+'</td></tr>';
  }).join('');
  rows += '<tr><td style="font-family:var(--condensed);font-weight:600">Avg Latency</td><td class="mono-val">'+oa.aggregate.avg_latency_ms+'ms</td><td class="mono-val">'+co.aggregate.avg_latency_ms+'ms</td></tr>';
  var perQ = (oa.per_query||[]).map(function(r,i){
    var cr = co.per_query[i]||{};
    return '<tr><td>'+(i+1)+'</td><td style="max-width:180px;word-break:break-word;font-size:12px">'+esc(r.query)+'</td><td class="mono-val" style="color:'+(r.ndcg_at_k>=(cr.ndcg_at_k||0)?'var(--openai)':'var(--text-2)')+'">'+r.ndcg_at_k.toFixed(4)+'</td><td class="mono-val" style="color:'+((cr.ndcg_at_k||0)>=r.ndcg_at_k?'var(--cohere)':'var(--text-2)')+'">'+((cr.ndcg_at_k||0)).toFixed(4)+'</td></tr>';
  }).join('');
  return '<div style="display:flex;flex-direction:column;gap:16px"><div class="card"><div class="card-header"><span class="card-title">OpenAI vs Cohere Benchmark</span><div style="display:flex;gap:8px"><span class="latency-pill">K='+k+'</span><span class="latency-pill">'+numQ+' queries</span></div></div><div class="card-body"><table class="data-table"><thead><tr><th>Metric</th><th style="color:var(--openai)">&#9679; OpenAI</th><th style="color:var(--cohere)">&#9679; Cohere</th></tr></thead><tbody>'+rows+'</tbody></table></div></div><div class="card"><div class="card-header"><span class="card-title">Per-Query NDCG</span></div><div style="overflow-x:auto"><table class="data-table"><thead><tr><th>#</th><th>Query</th><th style="color:var(--openai)">OAI NDCG</th><th style="color:var(--cohere)">Cohere NDCG</th></tr></thead><tbody>'+perQ+'</tbody></table></div></div></div>';
}

// --- COLLECTIONS ---
async function loadCollections() {
  try { var data = await apiFetch('/api/collections'); document.getElementById('collectionsOutput').innerHTML = renderCollections(data); attachClearHandlers(); }
  catch(e) { document.getElementById('collectionsOutput').innerHTML = '<div class="error-box">' + esc(e.message) + '</div>'; }
}

function renderCollections(data) {
  function renderColl(key, info, color, label) {
    var pts = info.points_count || 0, hasData = pts > 0;
    return '<div class="coll-box" style="margin-bottom:16px"><div class="coll-box-header"><div style="display:flex;align-items:center;gap:10px"><span class="model-badge ' + key + '">' + label + '</span><span style="font-family:var(--condensed);font-weight:600;font-size:14px;color:' + color + '">' + info.name + '</span></div><div style="display:flex;align-items:center;gap:8px"><div class="coll-status"><div class="coll-dot ' + (hasData?'green':'') + '"></div>' + (hasData?'Active':'Empty') + '</div><button class="btn btn-danger" data-clear="' + key + '" style="padding:6px 12px;font-size:12px">Clear</button></div></div><div style="padding:14px 18px"><div class="stat-row"><span class="stat-key">Points (chunks)</span><span class="stat-val">' + pts.toLocaleString() + '</span></div><div class="stat-row"><span class="stat-key">Indexed vectors</span><span class="stat-val">' + (info.vectors_count||0).toLocaleString() + '</span></div><div class="stat-row"><span class="stat-key">Status</span><span class="stat-val">' + (info.status||'unknown') + '</span></div><div class="stat-row"><span class="stat-key">Error</span><span class="stat-val" style="color:var(--danger)">' + (info.error||'&mdash;') + '</span></div></div></div>';
  }
  return '<div style="max-width:640px">' + renderColl('openai',data.openai,'var(--openai)','OpenAI') + renderColl('cohere',data.cohere,'var(--cohere)','Cohere') + '<div style="display:flex;gap:10px"><button class="btn btn-danger" data-clear="both">Clear All Collections</button><button class="btn btn-secondary" id="refreshCollBtn">Refresh</button></div></div>';
}
...
function attachClearHandlers() {
  document.querySelectorAll('[data-clear]').forEach(function(btn) {
    btn.addEventListener('click', async function() {
      ...
      if (!confirm('Clear ' + model + ' collection(s)? Cannot be undone.')) return;
      try { await apiFetch('/api/collections/' + model, {method:'DELETE'}); showToast('Cleared ' + model + ' collection(s)','success'); await loadCollections(); }
      catch(e) { showToast(e.message,'error'); }
    });
  });
  var r = document.getElementById('refreshCollBtn'); if (r) r.addEventListener('click', loadCollections);
}
